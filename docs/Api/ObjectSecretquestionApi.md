...
```php
secretquestionGetAutocompleteV2($sSelector, $eFilterActive, $sQuery, $acceptLanguage): \eZmaxAPI\Model\SecretquestionGetAutocompleteV2Response
```
...
try {
    $result = $apiInstance->secretquestionGetAutocompleteV2($sSelector, $eFilterActive, $sQuery, $acceptLanguage);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ObjectSecretquestionApi->secretquestionGetAutocompleteV2: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\eZmaxAPI\Model\SecretquestionGetAutocompleteV2Response**](../Model/SecretquestionGetAutocompleteV2Response.md)
