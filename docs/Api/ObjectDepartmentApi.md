...
```php
departmentGetAutocompleteV1($sSelector, $eFilterActive, $sQuery, $acceptLanguage): \eZmaxAPI\Model\CommonGetAutocompleteV1Response
```
...
try {
    $result = $apiInstance->departmentGetAutocompleteV1($sSelector, $eFilterActive, $sQuery, $acceptLanguage);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ObjectDepartmentApi->departmentGetAutocompleteV1: ', $e->getMessage(), PHP_EOL;
}
```
