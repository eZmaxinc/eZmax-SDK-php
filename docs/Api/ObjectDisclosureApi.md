...
```php
disclosureGetListV1($eOrderBy, $iRowMax, $iRowOffset, $acceptLanguage, $sFilter): \eZmaxAPI\Model\DisclosureGetListV1Response
```
...
try {
    $result = $apiInstance->disclosureGetListV1($eOrderBy, $iRowMax, $iRowOffset, $acceptLanguage, $sFilter);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ObjectDisclosureApi->disclosureGetListV1: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\eZmaxAPI\Model\DisclosureGetListV1Response**](../Model/DisclosureGetListV1Response.md)
