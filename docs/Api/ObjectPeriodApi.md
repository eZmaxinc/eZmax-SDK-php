...
```php
periodGetAutocompleteV2($sSelector, $eFilterActive, $sQuery, $acceptLanguage): \eZmaxAPI\Model\PeriodGetAutocompleteV2Response
```
...
try {
    $result = $apiInstance->periodGetAutocompleteV2($sSelector, $eFilterActive, $sQuery, $acceptLanguage);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ObjectPeriodApi->periodGetAutocompleteV2: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\eZmaxAPI\Model\PeriodGetAutocompleteV2Response**](../Model/PeriodGetAutocompleteV2Response.md)
