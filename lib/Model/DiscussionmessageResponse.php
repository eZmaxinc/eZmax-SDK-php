...
class DiscussionmessageResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($tDiscussionmessageContent) && (!preg_match("/(*UTF8)^[\\s\\S]{0,65535}$/", ObjectSerializer::toString($tDiscussionmessageContent)))) {
            //throw new \InvalidArgumentException("invalid value for \$tDiscussionmessageContent when calling DiscussionmessageResponse., must conform to the pattern /^[\\s\\S]{0,65535}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($tDiscussionmessageContent)?'null':'"'.$tDiscussionmessageContent.'"')." for tDiscussionmessageContent when calling DiscussionmessageResponse., must conform to the pattern /^[\\s\\S]{0,65535}$/.");
        }
