...
class CustomerAutocompleteElementResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (($fkiDepartmentID < 0)) {
            //throw new \InvalidArgumentException('invalid value for $fkiDepartmentID when calling CustomerAutocompleteElementResponse., must be bigger than or equal to 0.');
            throw new \InvalidArgumentException('invalid value '.(is_null($fkiDepartmentID)?'null':'"'.$fkiDepartmentID.'"').' for fkiDepartmentID when calling CustomerAutocompleteElementResponse., must be bigger than or equal to 0.');
        }
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sCustomerCode) && (!preg_match("/(*UTF8)^.{0,6}$/", ObjectSerializer::toString($sCustomerCode)))) {
            //throw new \InvalidArgumentException("invalid value for \$sCustomerCode when calling CustomerAutocompleteElementResponse., must conform to the pattern /^.{0,6}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sCustomerCode)?'null':'"'.$sCustomerCode.'"')." for sCustomerCode when calling CustomerAutocompleteElementResponse., must conform to the pattern /^.{0,6}$/.");
        }
