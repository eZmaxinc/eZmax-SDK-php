...
class BrandingRequestV2 implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($fkiDomainID) && ($fkiDomainID > 255)) {
            //throw new \InvalidArgumentException('invalid value for $fkiDomainID when calling BrandingRequestV2., must be smaller than or equal to 255.');
            throw new \InvalidArgumentException('invalid value '.(is_null($fkiDomainID)?'null':'"'.$fkiDomainID.'"').' for fkiDomainID when calling BrandingRequestV2., must be smaller than or equal to 255.');
        }
        //if (($fkiDomainID < 0)) {
        if (!is_null($fkiDomainID) && ($fkiDomainID < 0)) {
            //throw new \InvalidArgumentException('invalid value for $fkiDomainID when calling BrandingRequestV2., must be bigger than or equal to 0.');
            throw new \InvalidArgumentException('invalid value '.(is_null($fkiDomainID)?'null':'"'.$fkiDomainID.'"').' for fkiDomainID when calling BrandingRequestV2., must be bigger than or equal to 0.');
        }
