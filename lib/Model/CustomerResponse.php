...
class CustomerResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($tCustomerComment) && (!preg_match("/(*UTF8)^.{0,16777215}$/", ObjectSerializer::toString($tCustomerComment)))) {
            //throw new \InvalidArgumentException("invalid value for \$tCustomerComment when calling CustomerResponse., must conform to the pattern /^.{0,16777215}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($tCustomerComment)?'null':'"'.$tCustomerComment.'"')." for tCustomerComment when calling CustomerResponse., must conform to the pattern /^.{0,16777215}$/.");
        }
