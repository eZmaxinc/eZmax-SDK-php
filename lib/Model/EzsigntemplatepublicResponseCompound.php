...
class EzsigntemplatepublicResponseCompound implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sEzsigntemplatepublicUrl) && (!preg_match("/(*UTF8)^https:\/\/[^\\s\/$.?#].[^\\s]*$/", ObjectSerializer::toString($sEzsigntemplatepublicUrl)))) {
            //throw new \InvalidArgumentException("invalid value for \$sEzsigntemplatepublicUrl when calling EzsigntemplatepublicResponseCompound., must conform to the pattern /^https:\/\/[^\\s\/$.?#].[^\\s]*$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sEzsigntemplatepublicUrl)?'null':'"'.$sEzsigntemplatepublicUrl.'"')." for sEzsigntemplatepublicUrl when calling EzsigntemplatepublicResponseCompound., must conform to the pattern /^https:\/\/[^\\s\/$.?#].[^\\s]*$/.");
        }
