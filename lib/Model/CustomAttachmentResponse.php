...
class CustomAttachmentResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sAttachmentMD5) && (!preg_match("/(*UTF8)^.{32}$/", ObjectSerializer::toString($sAttachmentMD5)))) {
            //throw new \InvalidArgumentException("invalid value for \$sAttachmentMD5 when calling CustomAttachmentResponse., must conform to the pattern /^.{32}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sAttachmentMD5)?'null':'"'.$sAttachmentMD5.'"')." for sAttachmentMD5 when calling CustomAttachmentResponse., must conform to the pattern /^.{32}$/.");
        }
