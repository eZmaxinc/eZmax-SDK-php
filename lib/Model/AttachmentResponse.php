...
class AttachmentResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sAttachmentName) && (!preg_match("/(*UTF8)^.{0,75}$/", ObjectSerializer::toString($sAttachmentName)))) {
            //throw new \InvalidArgumentException("invalid value for \$sAttachmentName when calling AttachmentResponse., must conform to the pattern /^.{0,75}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sAttachmentName)?'null':'"'.$sAttachmentName.'"')." for sAttachmentName when calling AttachmentResponse., must conform to the pattern /^.{0,75}$/.");
        }
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sAttachmentMD5) && (!preg_match("/(*UTF8)^.{32}$/", ObjectSerializer::toString($sAttachmentMD5)))) {
            //throw new \InvalidArgumentException("invalid value for \$sAttachmentMD5 when calling AttachmentResponse., must conform to the pattern /^.{32}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sAttachmentMD5)?'null':'"'.$sAttachmentMD5.'"')." for sAttachmentMD5 when calling AttachmentResponse., must conform to the pattern /^.{32}$/.");
        }
