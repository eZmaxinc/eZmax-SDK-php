...
class ContactRequestCompound implements ModelInterface, ArrayAccess, \JsonSerializable
{
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sContactFirstname) && (!preg_match("/(*UTF8)^.{1,20}$/", ObjectSerializer::toString($sContactFirstname)))) {
            //throw new \InvalidArgumentException("invalid value for \$sContactFirstname when calling ContactRequestCompound., must conform to the pattern /^.{1,20}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sContactFirstname)?'null':'"'.$sContactFirstname.'"')." for sContactFirstname when calling ContactRequestCompound., must conform to the pattern /^.{1,20}$/.");
        }
	...
	//Openapi doesn't allow to set a variable to null when it's defined as Non-nullable even if it is the normal way of unsetting a variable
        ...
        if (!is_null($sContactLastname) && (!preg_match("/(*UTF8)^.{1,25}$/", ObjectSerializer::toString($sContactLastname)))) {
            //throw new \InvalidArgumentException("invalid value for \$sContactLastname when calling ContactRequestCompound., must conform to the pattern /^.{1,25}$/.");
            throw new \InvalidArgumentException("invalid value ".(is_null($sContactLastname)?'null':'"'.$sContactLastname.'"')." for sContactLastname when calling ContactRequestCompound., must conform to the pattern /^.{1,25}$/.");
        }
